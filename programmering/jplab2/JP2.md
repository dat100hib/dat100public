...
I tillegg er der en **obligatoriske oppgave** O1 der dere skal opprettet en brukerkonto på GitHub. Denne skal dere bruke etterhvert og også får å levere inn koden for neste obligatoriske oppgave. Detaljer for hva dere skal levere i O1-oppgaven samt frist finner dere på Canvas.
...
Generelt for et heltall `n` kan vi bruke `n % 2` til å avgjøre om verdien av `n` er et partall eller et oddetal ved å se på rest ved divisjon med `2` dvs. resultatet av `n % 2`. Hvis det er `0` er verdien av `n` et partall og om resultat er `1` er det et oddetal. 
...
Test programmet for ulike tildeling av heltall til `n`.

### Oppgave G2 - Switch-valgsetninger

Bruk en Java program som bruker en switch-setning til å finne ut gitt en nummer på en dagen i uken (1,2, .. 7) hvilken ukedag det svarer til.
...
Test programmet for ulike tildeling av verdier til variablen `nd`.
...
### Oppgave B2 (Gruppearbeid) - Datatyper
...
Sålangt i emnet har vi kun brukt GitHub for å kunne lage en åpen oppbevaringsplass i skyen: https://github.com/dat100hib/dat100public der vi kan legge ut oppgaver for programmeringslab og der dere kan gå tilgang til kode-eksempler fra undervisningen. 

Etterhvert i emnet skal vi skal vi bruke mer av funksjonaliteten som Git og GitHub tilbyr for at dere kan levere inn kode, få tilgang til startkode for oppgaver og slik dere kan jobbe sammen om koding på en strukturet måte.

Første steg for at dere skal komme igang med mer avansert bruk av Git er å opprettet en konto samt en første egen oppbevaringsplass som skal brukes for at dere kan levere inn Java-koden for den obligatoriske innleveringen som kommer på neste programmeringslab.
...
2. Velg *New* for å opprette et nytt repository. Under `General` gi det navnet `dat100-innlevering-1` og under `Configuration` velg *Add a README file* og velg *Java* under *Add .gitignore*

3. Trykk på *Create Repository* og du har nå opprettet et repository og bildet på skjermen skal være tilsvarende nedenfor.

4. Kopier linken til oppbevaringsplassen i nettleseren og lever denne inn via Canvas som svar på oppgaven.
